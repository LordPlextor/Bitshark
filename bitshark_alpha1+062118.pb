; *******************************************
; *** Bitshark Encryption Algorithm
; *** (c)2018 by M.Schmelter.
; *** Compiler: Purebasic 5.62 (64-Bit)/Linux
; *** Status: pre-alpha stage
; *******************************************

UseSHA3Fingerprint()

; **** Array-Dimensionierung der S-Box-Schlüssel (18 Runden á 8 Schlüssel á 32-Bit) *******
Global Dim sbox_keys.i(17, 7)       ; = 144 Rundenschlüssel
; *****************************************************************************************
Global k0.s, k1.s, k01.s, hex_res.s, char_res.s
Global plaintext.s = "TESTtestTESTtest"

Global Dim s_box.l(3,255)
Global plainbytes.s
Global Dim plain_32.s(3)
Global Dim p_v.i(3)
Global Dim result.i(3)

; ******************** Initialisierung der 32-Bit Rundenschlüssel *************************
Procedure Init_Keys(iv.s)
  tmp_key.s = iv.s
   For y = 0 To 8
     tmp_key.s = StringFingerprint(tmp_key.s, #PB_Cipher_SHA3, 512)
         For i=1 To 128 Step 8
             sbox_keys.i(c, j) = Val("$" + Mid(tmp_key.s, i, 8))
             j + 1
             If j > 7
               c + 1
               j = 0
   ...
   k01.s = StringFingerprint(tmp_key.s, #PB_Cipher_SHA3, 256)
   k0.s = Left(k01.s, 32)                ; Erzeugung von zwei 128 Bit Teilschlüsseln
   k1.s = Right(k01.s, 32)
...
Procedure Encrypt()
  ...
  For i=1 To 16
    plainbytes.s = plainbytes.s + Hex(Asc(Mid(plaintext.s, i, 1)), #PB_Byte)
  Next i
  plain_32.s(0) = "$" + Mid(plainbytes.s, 1, 8)
  plain_32.s(1) = "$" + Mid(plainbytes.s, 9, 8)
  plain_32.s(2) = "$" + Mid(plainbytes.s, 18, 8)
  plain_32.s(3) = "$" + Mid(plainbytes.s, 19, 8)
  
  For j=0 To 3
    p_v.i(j) = Val(plain_32.s(j))
    result.i(j) = p_v.i(j) ! sbox_keys.i(0, j)
    hex_res.s = hex_res.s + result.i(j) 
  Next j
  
  For z=1 To Len(hex_res.s) Step 2
    char_res.s = char_res.s + Chr(Val(Mid(hex_res.s, z, 2)))
  Next z
   
  
EndProcedure

Init_Keys("Passphrase")            ; Passphrase als Initialisierungsvektor IV übergeben
Encrypt()
